...
#warning Including deprecated SubDetId values.
    ...
    virtual SubDetId GetSubDetector() const;

    ///Check if this channel is a FGD channel.
    virtual Bool_t IsFGDChannel() const;

    ///Check if a UInt_t is a FGD channel id, without the user having to
    ///construct a channel id.
    static Bool_t IsFGDChannel(UInt_t code) {
        return TChannelId(code).IsFGDChannel();
    }

    ///Check if this channel is a TFB channel.
    virtual Bool_t IsTFBChannel() const;

    ///Check if a UInt_t is a TFB channel id, without the user having to
    ///construct a channel id.
    static Bool_t IsTFBChannel(UInt_t code) {
        return TChannelId(code).IsTFBChannel();
    }

    ///Check if this channel is a TFB channel.
    virtual Bool_t IsTPCChannel() const;

    ///Check if a UInt_t is a TPC channel id, without the user having to
    ///construct a channel id.
    static Bool_t IsTPCChannel(UInt_t code){
        return TChannelId(code).IsTPCChannel();
    }

    ///Check if this channel is an SFG channel.
    virtual Bool_t IsSFGChannel() const;

    ///Check if a UInt_t is an SFG channel id, without the user having to
    ///construct a channel id.
    static Bool_t IsSFGChannel(UInt_t code){
        return TChannelId(code).IsSFGChannel();
    }

    ///Check if this channel is a HAT channel.
    virtual Bool_t IsHATChannel() const;

    ///Check if a UInt_t is a HAT channel id, without the user having to
    ///construct a channel id.
    static Bool_t IsHATChannel(UInt_t code){
        return TChannelId(code).IsHATChannel();
    }

    ///Check if this channel is a TOF channel.
    virtual Bool_t IsTOFChannel() const;

    ///Check if a UInt_t is a TOF channel id, without the user having to
    ///construct a channel id.
    static Bool_t IsTOFChannel(UInt_t code){
        return TChannelId(code).IsTOFChannel();
    }
...
#warning Using deprecated AsInt.
    ...
    virtual UInt_t AsInt() const METHOD_DEPRECATED {return fChannelId;}
#endif

    ///Get the internal UInt_t representation
    virtual UInt_t AsUInt() const {return fChannelId;}
